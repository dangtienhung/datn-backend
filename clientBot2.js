...
app.get("/Service/bot_history", async (req,res)=>{
  //thông tin từ cookie
     ...
     ns.forEach(val=>{
          ...
          if(MinTimeH<(val['createdAt'].toISOString()).substring(11,13)*1)MinTimeH=(val['createdAt'].toISOString()).substring(11,13)*1;
          if(MaxTimeH>(val['createdAt'].toISOString()).substring(11,13)*1)MinTimeH=(val['createdAt'].toISOString()).substring(11,13)*1;
    ...
    // Similar
     ...
     ProductsList.forEach(p=>{
          //setting
          if(Similar(p.name,Pstring)>0)SmList.unshift({id:p.id,name:p.name,priority:(Similar(p.name,Pstring)).toFixed(3)+"%"})
          // 50%
