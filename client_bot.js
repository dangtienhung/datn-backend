...
function isEmptyObject(obj) {
  for (let key in obj) {
    if (obj.hasOwnProperty(key)) {
      return false; // Nếu có ít nhất một thuộc tính, object không trống
    }
  }
  return true; // Nếu không có thuộc tính nào, object trống
}
...
app.get('/Service/bot_history', async (req, res) => {
  ...
  if (refreshTokenCookie) {
    ...
    const url = 'http://localhost/bot?n=' + OrderHistory;
    axios
      .get(url)
      .then(async (response) => {
        var ns = response.data[0];
        var ns1 = response.data[1][JWT_id];
        //Giờ
        var MinTimeH = response.data[0][0]['createdAt'].substring(11, 13) * 1;
        var MaxTimeH = response.data[0][0]['createdAt'].substring(11, 13) * 1;
        //ngày
        var AvDay = 0;
        var FirstDay = response.data[0][0]['createdAt'].substring(5, 7) * 1;
        //tiền
        var total = response.data[0][0]['total'];
        //lặp lại
        var ReOrder = {};
        //mua thêm
        var toppings = '';
        ...
        ns.forEach((val) => {
          ...
          if (val.user == JWT_id) {
            ...
            if (MinTimeH < val['createdAt'].substring(11, 13) * 1)
              MinTimeH = val['createdAt'].substring(11, 13) * 1;
            if (MaxTimeH > val['createdAt'].substring(11, 13) * 1)
              MinTimeH = val['createdAt'].substring(11, 13) * 1;
            AvDay = (-FirstDay + val['createdAt'].substring(5, 7) * 1) / 2;
...
server.listen(4000, (req, res) => {
  console.log(chalk.blue('Bot start success !'));
});
